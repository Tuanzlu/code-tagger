...
class code_DB(DB):
    def __init__(self, db_path = 'data/code.sqlite'):
        ...
        self.init_table_sql = '''
        CREATE TABLE IF NOT EXISTS "Code" ("_id" INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL  UNIQUE , "userID" VARCHAR, "code" VARCHAR, "code_name" VARCHAR, "time" VARCHAR);
        '''
    ...
    def oneUserModifyCode(self, userID, c_name, c_new):
        ...
        self.execute("UPDATE Code SET time='{}' WHERE userID='{}' AND code_name='{}'".format(t, userID, c_name))
        return self.cursor.lastrowid

    def selectOneCode(self, userID,c_name):
        self.cursor.execute("SELECT userID, code, code_name, time FROM Code WHERE userID = '{}' AND code_name='{}'".format(userID,c_name))
        outs = self.cursor.fetchall()
        return outs
